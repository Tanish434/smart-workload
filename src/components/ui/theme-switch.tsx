...
function applyWithTransition(
  ...
  if (typeof document === "undefined" || !document.startViewTransition || window.innerWidth > 1800) {
    apply();
    return;
  }
  const { x, y } = origin;
  ...
  if (!mounted) {
    ...
    return (
      <div
        aria-hidden
        className={cn(
          "w-9 h-9 rounded-full bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700",
          className,
        )}
      />
    );
  ...
  return (
    <motion.button
      ...
      className={cn(
        ...
        "bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 cursor-pointer",
        "text-slate-700 dark:text-slate-200 outline-none",
        "hover:bg-slate-200/70 dark:hover:bg-slate-700/80 transition-colors",
